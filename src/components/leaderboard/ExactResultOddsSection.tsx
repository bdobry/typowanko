...
export function ExactResultOddsSection({
  ...
  const filteredMatches = showHitsOnly ? matches.filter((match) => match.hits.length > 0) : matches;
  const visibleMatches = displayMode === 'all'
    ? filteredMatches
    : filteredMatches.slice(0, TOP_LIMIT);
  const hasMore = filteredMatches.length > TOP_LIMIT;
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <label className="inline-flex h-8 items-center gap-2 rounded-md border border-gray-200 bg-white px-2.5 text-xs font-semibold text-gray-700 transition-colors hover:bg-gray-50">
            <input
              type="checkbox"
              checked={showHitsOnly}
              onChange={(event) => setShowHitsOnly(event.target.checked)}
              className="h-3.5 w-3.5 rounded border-gray-300 text-green-600 focus:ring-green-600"
            />
            Pokaż tylko trafione
          </label>
          {hasMore && (
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        {visibleMatches.length > 0 ? (
          ...
          <p className="px-4 py-6 text-center text-sm text-gray-400">
            Brak trafionych dokładnych wyników.
          </p>
        )}
      </div>
    </div>
